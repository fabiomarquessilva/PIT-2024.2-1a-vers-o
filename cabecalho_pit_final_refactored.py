...
class PITApp:
    def __init__(self, root):
        ...
        self.observations = ""  # Observações do docente
...
"""
    ...
    def show_monitoring_section(self):
        self.show_section_form(
            ...
            next_action=self.show_tcc_section,
            section_key="monitoring"
        )

    def show_tcc_section(self):
        self.show_section_form(
            title="Orientação de TCC",
            labels=["Natureza", "Título da Pesquisa", "CH Semanal"],
            next_action=self.show_production_section,
            section_key="tcc"
        )
    
    def show_production_section(self):
        self.show_section_form(
            title="Produção Intelectual",
            labels=["Natureza", "Título da Produção", "CH Semanal"],
            next_action=self.show_other_activities_section,
            section_key="production"
        )

    def show_other_activities_section(self):
        self.show_section_form(
            title="Outras Atividades",
            labels=["Atividade", "CH Semanal"],
            next_action=self.show_remote_work_section,
            section_key="others"
        )

    def show_remote_work_section(self):
        self.show_section_form(
            title="Atividades Pedagógicas Remotas",
            labels=["Período/Data", "Atividade", "Ferramenta", "CH Semanal"],
            next_action=self.show_final_summary,
            section_key="remote_work"
        )

    def show_section_form(self, title, labels, next_action, section_key):
        ...
        header_label = tk.Label(header_frame, text=title, font=("Arial", 16, "bold"), fg="white", bg="#003366", justify="center")
        header_label.pack()
        ...
        for i, label in enumerate(labels):
            ...
            entry = ttk.Entry(form_frame, width=40 if i == 0 else 10)
            entry.grid(row=i, column=1, pady=5, padx=5)
            self.data_entries[label] = entry

        add_button = ttk.Button(form_frame, text="Adicionar à Tabela", command=lambda: self.add_to_table(section_key, labels))
        ...
        self.tree = ttk.Treeview(self.table_frame, columns=labels, show="headings", height=10)
        for label in labels:
            self.tree.heading(label, text=label)
            self.tree.column(label, width=200 if label != "CH Semanal" else 100)
        self.tree.pack(fill="both", expand=True)

        # Botão para a próxima seção
        next_button = ttk.Button(self.root, text="Próxima Seção", command=next_action)
